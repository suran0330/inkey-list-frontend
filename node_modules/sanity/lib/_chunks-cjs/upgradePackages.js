...
async function upgradePackages(options, context) {
  const {
    ...
    cwd: workDir,
    stdio: "inherit"
  }, upgradePackageArgs = packages.map((pkg) => pkg.join("@")), npmArgs = ["upgrade", "--legacy-peer-deps", ...upgradePackageArgs];
  let result;
  if (packageManager === "npm")
    output.print(`Running 'npm upgrade ${npmArgs.join(" ")}'`), result = await execa__default.default("npm", npmArgs, execOptions);
